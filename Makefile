...
MYGCCFLAGS := '-O3 -m32 -march=native -msse -msse2 -msse3 -ffast-math $(OSCCTRL)'
else
#MYGCCFLAGS := '-O3 -msse -msse2 -msse3 -ffast-math -Wno-extra-tokens -Wno-return-type -Wno-unused-variable $(OSCCTRL)'
MYGCCFLAGS := '-O3 -fpermissive -msse -msse2 -msse3 -ffast-math  $(OSCCTRL)'
endif
...
# SCCFLAGS := $(MYGCCFLAGS:'=) `pkg-config --cflags libscsynth`'
# SCCFLAGS := $(MYGCCFLAGS:=) `pkg-config --cflags libscsynth`
# SCCFLAGS := $(MYGCCFLAGS:=) -lscsynth
...
clean :
	...
	rm *.dsp &
	rm *.mdx &
	# EJB commented this out:  $(MAKE) -f Makefile.mathdoc clean
